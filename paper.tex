...
\newcommand{\compatibletailhead}[2]{\ensuremath{#1\;R\;#2}}
...
representations to several known cast representations
(\cite{Siek:2010:TWB:1706299.1706342,siek2012interpretations,Siek:2015:BCT:2737924.2737968})
and believe this framework
is also applicable to other cast representations (\cite{Garcia:2013:CTB:2500365.2500603,siek2012interpretations}).
We also present a new cast representation, hypercoercions, that is more friendly
to mechanized meta-theory and consumes less memory than coercions
in normal forms.
...
\todo[inline]{JS: In the above you've lumped proxy-based and blame
  ...
  \citet{Breazu-Tannen:1990vc} is the origin of proxy-based
  casts. (Who knows, there could be an even earlier paper for
  proxy-based casts, but that's the earliest one I know.)
  \citet{herman2010space} cited the wrong paper for function proxies,
  perhaps because they didn't know about the decade earlier
  \citet{Breazu-Tannen:1990vc}. Also, \cite{findler2002contracts}
  should have cited \citet{Breazu-Tannen:1990vc} but did not. Perhaps
  because they did not consider that papers about ``casts'' to be
  closely related to their paper about ``contracts''.  Or perhaps they
  too did not know about \citet{Breazu-Tannen:1990vc}.  Authors make
  mistakes all the time regarding related work, and this is just yet
  another example of that. For that matter, I should have cited
  \citet{Breazu-Tannen:1990vc} in \citet{siek2006gradual} but did not,
  probably because I had learned about it from the TAPL textbook
  (section 15.6) and therefore thought it was so standard that it
  didn't need a citation.  But it would have been better for me to
  cite it.}
...
\todo[inline]{KC: This paragraph suggests that
\cite{siek2009exploring} gives the definitional semantics.
\cite{siek2009exploring}'s is coercion-based.
It makes more sense to me to use a type-based semantics as the definition.
Actually in my mind \cite{siek2012interpretations} is definitional.}
...
\todo[inline]{JS: say something about the difference between the above
  and what is in this paper. That's the whole point of the related
  work section from the eyes of the reviewer.}
\todo[inline]{KC: I might have some misconcept about the related work section.
You might have felt this, haha. Now I think this section is targeting the
following scenarios: 
...
\todo[inline]{JS: yes, and make sure to
  give very explicit answers to these questions. It should feel to you
  like you're hitting the reader over the head with the most obvious
  things (to you). The above paragraph is currently insufficient
  because it doesn't emphasize what scientific constributions are new
  to our paper compared to those prior papers on coercions in normal
  form.}
\todo[inline]{KC: I rewrote this section. Please check again.}

\subsection{Dynamics semantics parameterized over cast representations}
\citet{siek2020mechanized} defines two dynamic semantics for 
cast calculi parameterized over cast representations. The first
semantics, namely CC, is \textit{not} space-efficient and
parameterized over $\mathrm{CastStruct}$. The author instantiates
CC to obtain the blame calculus ($\lambda B$) in 
\cite{Siek:2010:TWB:1706299.1706342}. 
The resulting semantics is analogous to our \LUDMachine.
The other semantics, namely SC, is designed to be space-efficient
and parameterized over $\mathrm{EfficientCastStruct}$. SC is analogous
to our \SMachine{C}. CC and SC give their users more control
over the structure of values. Our \CMachine{B} machine
insists that the cast on the top of a value must be a cast between
compound types (specifically, functions and pairs).
In contrast, users of their interfaces define
a predicate on casts called $\mathrm{Inert}$ and any inert cast can
be a part of a value. Their system doesn't include theorems relating
space-inefficient cast calculi and space-efficient ones, which
is one of the major goals of our framework.
...
coercions in \citet{herman2010space}, using correctness-preserving
transformation.
...
we also use the bisimulation technique.
...
\end{array}
\]
In Agda code we use the monad bind operator instead.
% We use 
...
\todo[inline]{KC: KC, you need to replace all uses of monad bind to let.}
...
\todo[inline]{KC: that is possibly only if we steal some pages from elsewhere.
The paper limit is 25 (excluding bib) and we have used 30 now.
https://etaps.org/2021/call-for-papers}
...
Subsection~\ref{sec:LazyD-Hypercoercions} implements the \CastADT{}
with the \LD{} hypercoercions and proves that \LD{} hypercoercions is
a Lazy D Cast ADT.  Subsection~\ref{sec:LazyUD-Hypercoercions} does
the same for \LUD{}.  Finally, we argue in
...
\todo[inline]{JS: Yes}
\todo[inline]{KC: Could you show me how to do this? I don't know exactly
how to write this kind of citations.}
...
for \LD\ Coercions in Normal Form, reported by Kuang-Chen Lu and
Deyaaeldeen Almahallawi. So it is useful to relax this restriction for
the \LD\ tracking strategy.
%
