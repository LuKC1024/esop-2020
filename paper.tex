...
\subsection{Dynamics parameterized over cast representations}
...
CC to obtain the blame calculus ($\lambda B$). 
...
to our \SMachine{C}. These two parameterized semantics give
users more control over the structure of values. Our \CMachine{B} machine
insists that the cast on the top of a value must be a function cast
or a product cast. In contrast, users of their interfaces define
...
prove the correctness of Lazy UD threesomes. To use threesomes for
...
Then they define a bisimulation relation connecting the blame calculus
and the threesome calculus and prove a bisimulation between them.
...
three calculi (i.e. $\lambda B$ (the blame calculus), $\lambda C$, and
$\lambda S$). $\lambda C$ uses coercions but doesn't normalize coercions
so is not space-efficient. $\lambda S$ uses coercions in normal form
and is space-efficient. To establish the correctness of $\lambda S$,
