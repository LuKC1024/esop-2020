...
\usepackage[disable]{todonotes} %% For \todo
...
\newcommand{\evaltoR}[3]{\ensuremath{\mathrm{eval}_{\CMachine{#1}}(#2)=#3}}
...
\begin{abstract}
% Background
Gradual typing combines dynamic typing and static typing in one
language to provide the advantages of both typing
disciplines. Implementing gradual typing with reasonable overhead,
however, is not trivial. Casts accumulate on higher-order values and
continuations, changing the overall space complexity. Several
space-efficient cast representations have been proposed, notably
coercions in normal forms and threesomes.
% Problem
The proofs of correctness of these cast representations are
challenging, involving a bisimulation between the language semantics
and a space-efficient implementation
\citep{Siek:2010:TWB:1706299.1706342,Siek:2015:BCT:2737924.2737968}.
The prospect of mechanizing these proofs is even more daunting.
% Solution
In this paper we present a framework in Agda that reduces the cost of
mechanized proofs of correctness for cast representations.

%% The abstract is too long... I wrote the following but I'm now
%% cutting it. No need for that much detail here. -Jeremy
%% We present
%% an abstract data type, the Cast ADT, that defines an interface for
%% cast representations and we specify which properties are necessary for
%% a Cast ADT to be correct. We then define an abstract machine
%% parameterized over Cast ADTs and prove its correctness.  A user of our
%% framework proves the correctness of their cast representation by
%% implementing the Cast ADT, proving that it satisfies the correctness
%% properties, and then instantiating our generic theorem.

%% A correct cast representations shall be proved not changing the overall
%% behavior of language implementations except saving space.
%% Existing proofs typically go through a 
%% % Problem
%% The similarity of these proofs suggests that people should be able
%% to prove correctness for a cast representation by proving it holds
%% certain properties.
%% % Solution
%% We present an abstract data type, namely Cast ADT,
%% that captures interface functions a cast representations typically 
%% provides, and an abstract machine parameterized over Cast ADTs.
%% Our abstract machine comes with generic theorems,
%% % Why is our solution a step forward
%% with which the correctness proofs can be done by just proving 
%% properties of a specific Cast ADT.
% %
% To isolate cast representations from the remaining part of 
% implementations, we present a abstract data type, namely Cast ADT,
% that captures interface functions a cast representations typically 
% includes, and an abstract machine parameterized over Cast ADTs.
% %
% With our generic abstract machine, the correctness proofs can be
% done by proving properties of a specific Cast ADT, which is simpler
% than proofs regarding the entire dynamic semantics.
%%%%%
% %
% But they are presented in
% different dynamic semantics, which make it difficult to compare the
% key differences between the cast representations.
% %
% We present a
% abstract data type, namely Cast ADT, that captures interface functions
% a cast representations typically includes, and an abstract machine
% parameterized over Cast ADTs.
% %
% The Cast ADT brings another benefit:
% space-efficient cast representations shall be proved not changing the
% overall behavior of language implementations except saving space.
% %
% With our generic abstract machine, these proofs can be done by proving
% properties of a specific Cast ADT, which is simpler than proofs
% regarding the entire dynamic semantics.
\todo[inline]{JS: The stuff above about ``make it difficult to compare''
  is a weak argument. I would recommend deleting that argument and
  instead focus on the benefits regarding the proofs of correctness.}
\todo[inline]{KC: updated.}

We apply our framework to obtain proofs of correctness for two
space-efficient cast representations from the literature
\citep{Siek:2010:TWB:1706299.1706342,siek2012interpretations,Siek:2015:BCT:2737924.2737968}
and believe this framework is also applicable to other cast
representations
\citep{Garcia:2013:CTB:2500365.2500603,siek2012interpretations}.
Finally, we present a new cast representation, named hypercoercions,
that is more friendly to mechanization and consumes less memory than
coercions in normal forms. We prove correctness for two variants of
hypercoercions using our framework.
...
and \cite{findler2002contracts}. My understanding is \cite{findler2002contracts}
extends the approach in \cite{breazu1991inheritance} with
blame labels.}
...
and proofs of correctness for cast representations
...
%% The following details are less important. -Jeremy
%% CC and SC give their users more control over the structure of
%% values. Our \CMachine{B} machine insists that the cast on the top of a
%% value must be a cast between compound types (specifically, functions
%% and pairs).  In contrast, users of their interfaces define a predicate
%% on casts called $\mathrm{Inert}$ and any inert cast can be a part of a
%% value.
...
\begin{array}{rcll}
  ...
  \begin{cases}
    ...
    [r/x]e & \text{otherwise}
  \end{cases}
    %% (\mlet{x}{(\error{l})}{e}) & = & \error{l} \\
    %% (\mlet{x}{a}{e}) & = & [a/x]e & \text{otherwise} \\ 
\end{array}
\]
...
\begin{figure}[tp]
	...
	\figreducetoH{\scont{v}{\Kcons{\Fcast{\ccast{S}{l}{T}}}{K}}}{
	    \mlet{u}{\applyCast{v}{\ccast{S}{l}{T}}}{\scont{u}{K}}
...
respect to \LD\ are in the injectable types and in \applyCast{v}{\ccast{S}{l}{T}},
in which a cast whose
...
\]
	...
	\fbox{$\applyCastName_{\mathtt{\BLUD}}(v,\ccast{S}{l}{T}) = r$}
	\[
	\begin{array}{rclr}
	\applyCastName(v, \ccast{\Tdyn}{l}{\Tdyn}) &=& \just{v} \\
	\applyCastName(v, \ccast{G}{l}{\Tdyn}) &=&
	\just{\vdyn{G}{v}} \\
	\applyCastName(v, \ccast{P}{l}{\Tdyn}) &=&
	\just{\vdyn{G}{\proxy{v}{\ccast{P}{l}{G}}}}
	\\ && \text{if } G \sim P, G \neq P \\  
% 	& \text{if } G \sim P, G \neq P \\  
	\applyCastName(\vdyn{G}{u}, \ccast{\Tdyn}{l}{H}) &=&          
	\project{u}{G}{l}{H}\\
	\applyCastName(\vdyn{G}{u}, \ccast{\Tdyn}{l}{Q}) &=&
	\mlet{v}{\project{u}{G}{l}{H}}{\proxy{v}{\ccast{H}{l}{Q}}}
% 	\project{u}{G}{l}{H} \mbind \lambda v. \proxy{v}{\ccast{H}{l}{Q}}
	\\
	&& \text{if } H \sim Q, H \neq Q \\
% 	& \text{if } H \sim Q, H \neq Q \\
	\applyCastName(v, \ccast{P}{l}{Q}) &=&
	\just{\proxy{v}{\ccast{P}{l}{Q}}}
	\\&& \text{if } P \smile Q \\
	\applyCastName(v, \ccast{P}{l}{Q}) &=&
	\error{l}
...
\todo[inline]{KC: is here the right place to mention that in Agda
code we use de Bruijn indexes?}
In the Agda code we use de Bruijn indices instead of named variables.
...
% Readers are encouraged to compare these two
% definitions with the corresponding $\applyCastName$ functions 
...
%\begin{lemma}[$\mathrm{mkCont}$ Preserves Bisimulation]
%	If \bisimK{K}{k}{S}{T} then
%	\[
%	\bisimK{K}{\mkCont{k}}{S}{T}.
%	\]
%\end{lemma}
%\begin{lemma}[$\mathrm{extCont}$ Preserves Bisimulation]
%	If $\bisimc{
%		(\ccast{S_1}{l_1}{S_2}\ldots\ccast{S_{n-1}}{l_{n-1}}{S_n})
%	}{c}{S_1}{S_n}$
...
A general description of the proof is in the Appendix.
...
\todo[inline]{JS: There should be some text regarding the following
two theorems.}
\todo[inline]{KC: Fixed.}

Lemma~\ref{lem:weak-bisimulation} implies that if a \CMachine{B} state $s_1$ transitions to a 
final state $s_2$ and \bisims{s_1}{s'_1}{T}, the state $s_1'$
will transition to a final state $s_2'$ that holds the same observable as $s_1'$. 
Instantiating Lemma~\ref{lem:weak-bisimulation} with Lazy D and Lazy UD blame strategy leads
to the next two theorems.
...
\todo[inline]{KC: should we cite the place where ``Prior presentations'' locate?}
...
$\mathrm{castToDyn}$ and $\mathrm{castFromDyn}$, is to ensure that each of them 
...
\begin{figure}[tp]
	...
	\figequalto{\denote{\hccc{h}{m}{t}}(v)}{
	    ...
	    \denote{t}_t(v)}}
	   % \mlet{v}{(\mlet{v}{\denote{h}(v)}{\denote{m}(v)})}{
	   % \denote{t}(v)}
...
\begin{figure}[tp]
  % removing fbox's to save space -Jeremy
%\fbox{$\goodgap{g}$}
\begin{gather*}
...
% Here composition $\compose{c_1}{c_2}$ makes the usual assumption that the
% target type of $c_1$ matches the source type of $c_2$. 
%
...
Agda is able to understand that \composegap{c}{g}{c} is
terminating. The reason might be that in \composegap{c}{g}{c} both
casts become smaller in recursive calls, although their position is
swapped in the function case. Readers who dislike non-structural
recursion could add a Boolean parameter to indicate the direction of
composition.
...
\begin{figure}[tp]
	...
	\end{array}
	\]
	\caption{Translation of \LD\ Hypercoercions}
	\label{fig:LazyD-Hypercoercions-translation}
\end{figure}

The definition of the translation function \translate{\ccast{S}{l}{T}}
is straightforward (\figref{fig:LazyD-Hypercoercions-translation}).
...
\figequalto{\denote{\hccc{h}{m}{t}}(v)}{
    \mlet{v}{\denote{h}_h(v)}{
    \mlet{v}{\denote{m}_m(v)}{\denote{t}_t(v)}}
...
\end{itemize}
The bits for blame labels represent an index into a table of blame
information.  If a program requires too many blame labels, then the
implementation can fallback to using the heap-allocated structure for
more coercions.
\end{itemize}
...
a parameterized abstract machine \SMachine{C}, a theorem stating that
...
\figref{fig:hypercoercion-statics} defines the static semantics of
hypercoercions.
...
\begin{figure}[tp]
	...
	\caption{Static semantics of hypercoercions}
	\label{fig:hypercoercion-statics}
\end{figure}

\todo[inline]{KC: The general idea of Lemma~\ref{lem:weak-bisimulation}}

Lemma~\ref{lem:weak-bisimulation} is proved by case analysis on \bisims{s_1}{s_1'}{T}.
Function call is the most interesting case. In this case the function value in
\CMachine{B}
must be of the form 
\[
v_1 = \vfuncc{
...
\]
By induction of the sequence of casts, we know \scont{v_2}{\Kcons{\FappOX{v_1}}{K}} will
transition to
\[
\begin{array}{l}
\mlet{v_2}{\applyCast{v_2}{\ccast{S_n}{\bar{l}}{S_{n-1}}}}{} \\
\ldots \\
\mlet{v_2}{\applyCast{v_2}{\ccast{S_2}{\bar{l}}{S_1}}}{} \\
\sexpr{e}{E[x:=v_2]}{
	\Kcons{\Fcast{\ccast{T_1}{l}{T_2}}}{
	    \ldots
	    \Kcons{\Fcast{\ccast{T_{n-1}}{l}{T_n}}}{K}}
}
\end{array}
\]
% Assuming \bisimv{v_1}{v_1'}{\Pfunc{S_n}{T_n}} and \bisimv{v_2}{v_2'}{S_n}.
As for \SMachine{C}, the function value must be of the form
\[
v_1' = \ecast{\vfunc{x}{S}{e}{E'}}{\Pfunc{c}{d}}
\]
where
\[
\begin{array}{l}
\bisimc{(\ccast{S_n}{\bar{l}}{S_{n-1}} \ldots \ccast{S_2}{\bar{l}}{S_1})}{c}{S_n}{S_1} \\
\bisimc{(\ccast{T_1}{l}{T_1} \ldots \ccast{T_{n-1}}{l}{T_n})}{d}{T_1}{T_n} \\
\end{array}
\]
By the definition of \applyContName\ we know \applyCont{v_2'}{\Kcons{\FappOX{v_1'}}{K}} will
transition to
\[
\begin{array}{l}
\mlet{v_2'}{\denote{c}(v_2')}{} \\
\sexpr{e}{E'[x:=v_2']}{\extCont{d}{K'}}
\end{array}
\]
Intuitively, we need to show the sequence of let bindings in \CMachine{B}
bisimulate the single let binding in \SMachine{C}, and that the two 
extended continuations bisimulate each other. The first part follows from 
the properties of Cast ADT (Definition~\ref{def:cast-rep}) and the premise that 
cast applications preserve the bisimulation relation. The second part
