...
\newcommand{\car}[1]{\ensuremath{\mathrm{fst}(#1)}}
\newcommand{\cdr}[1]{\ensuremath{\mathrm{snd}(#1)}}
...
imprecise type annotations. A gradual type checker allows programmers to cast 
...
and Lazy UD threesomes. 
We believe the framework is also applicable to other cast representations.
\todo[inline]{KC: (about the p above) I want to put the illustration part on the same github repo, adding description as
Agda comments. }

\todo[inline]{KC: (about the p below) Actually I think twosome's composition is structurally recursive but threesomes' is not}
...
(Section~\ref{sec:hypercoercions}), named
...
\todo[inline]{KC: how to make \textbackslash citet work?}
...
we review coercions and their normal forms (Subsection~\ref{sec:coercions-in-normal-form}), which
...
\todo[inline]{KC: I want to give a brief discussion on inert v.s. active pairs.}
...
\todo[inline]{KC: In the latex file, below are two versions of a figure.
One is the same as in our WGT paper. The other uses Z as the meta variable of sequences of 
source-code casts, which is similar to the blame-and-coercion paper.}
...
\begin{lemma}[Weak Bisimulation between \RMachine{B} and \SMachine{C}] \label{lem:weak-bisimulation}
Assume
\begin{enumerate}
    ...
    \item $s_1$ is a state in \RMachine{B} and
